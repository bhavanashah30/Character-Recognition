...
\pard\tx2520\f1 function  c = fchcode(b, conn, dir) \par
...
% terminates. \par
if any(abs(DEL(:, 1)) > 1) | any(abs(DEL(:, 2)) > 1); \par
   error('The input curve is broken or points are out of order.') \par
end \par
 \par
% Create a single index vector using the formula described above. \par
z = 4*(DEL(:, 1) + 2) + (DEL(:, 2) + 2); \par
 \par
...
if conn == 4 \par
   val = find(fcc == 1 | fcc == 3 | fcc == 5 | fcc ==7 ); \par
   if isempty(val) \par
      fcc = fcc./2; \par
   else \par
      warning('The specified 4-connected code cannot be satisfied.') \par
   end \par
end \par
 \par
...
c.mm = minmag(fcc); % See below for function minmag. \par
 \par
% Obtain the first difference of fcc \par
c.diffmm = codediff(c.mm, conn); \par
 \par
%-------------------------------------------------------------------% \par
function cr = coderev(fcc) \par
...
ind1 = find(0 <= cr & cr <= 3); \par
ind2 = find(4 <= cr & cr <= 7); \par
cr(ind1) = cr(ind1) + 4; \par
cr(ind2) = cr(ind2) - 4; \par
 \par
%-------------------------------------------------------------------% \par
function z = minmag(c) \par
...
J = 0; \par
A = zeros(length(I), length(c)); \par
for k = I; \par
   J = J + 1; \par
   A(J, :) = circshift(c,[0 -(k-1)]); \par
end \par
 \par
...
for k = 2:N \par
   ...
   if length(J)==1 \par
     ...
     \par
%-------------------------------------------------------------------% \par
function d = codediff(fcc, conn) \par
